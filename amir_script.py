...
for ts in [1, 2, 3, 4, 5, 6, 7]:
    ...
    overlap_RIDs = dm.generate_overlapRIDs(
        [RIDs_cog, RIDs_csf, RIDs_MRI, RIDs_demo]
    )
    acc = []
    ...
    for i in range(5):
        ...
        for j in range(5):  # 5-fold cross validation
            ...
            m = MMRNN(classifier=RandomForestClassifier(n_estimators=100))
            ...
            with tf.compat.v1.variable_scope("single_run"):
