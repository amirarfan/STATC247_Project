...
class MMRNN:
    ...
    def __init__(self, classifier=LogisticRegression(penalty="l1", C=80, class_weight="balanced")):
        self.components = {}
        self.IDs = {}
        self.data = {}
        self.label = {}
    ...
    def append_component(
        self, name, n_input, n_hidden, max_seq_len, cell=None, optimizer=None
    ):
        ...
        with tf.variable_scope(name):
            new_component = RNNComponent(
                n_hidden, max_seq_len, x, y, seqlen, cell, optimizer
            )
    ...
    def single_modal_training(self, name, batch_size):
        ...
        trainingIDs = self.IDs[name][
            ~pd.Series(self.IDs[name]).isin(self.test_overlapIDs)
        ]
    ...
    def predict_test(self):
        ...
        for name in self.IDs:
            component = self.components[name]
            IDs = self.IDs[name]
            data = self.pickup_by_ID(
                IDs, self.data[name], self.test_overlapIDs
            )
            y = self.pickup_by_ID(IDs, self.label[name], self.test_overlapIDs)
            seqlen = self.pickup_by_ID(
                IDs, self.seqlen[name], self.test_overlapIDs
            )
    ...
    def predict(self, IDs, data, seqlen, overlapIDs):
        predictor = self.integrative_feature_extraction(
            IDs, data, seqlen, overlapIDs
        )
    ...
    def integrative_feature_extraction(self, IDs, data, seqlen, overlapIDs):
        ...
        for name in self.IDs:
            ...
            aligned_seqlen = self.pickup_by_ID(
                IDs[name], seqlen[name], overlapIDs
            )
            bucket.append(
                component.extract_output(
                    self.sess, aligned_data, aligned_seqlen
                )
            )
