...
class RNNComponent:
    def __init__(
        self, n_hidden, max_seq_len, x, y, seqlen, cell=None, optimizer=None
    ):
        if cell == None:
        ...
        self.weights = tf.Variable(tf.random.normal([n_hidden, 1]))
        self.biases = tf.Variable(tf.random.normal([1]))

        x = tf.unstack(x, axis=1)

        self.outputs, self.states = tf.compat.v1.nn.static_rnn(
            self.cell, x, dtype=tf.float32, sequence_length=seqlen
        )
        self.outputs = tf.stack(self.outputs)
        self.outputs = tf.transpose(a=self.outputs, perm=[1, 0, 2])
        ...
        self.outputs = tf.gather(
            tf.reshape(self.outputs, [-1, n_hidden]), index
        )
        self.pred = tf.nn.softmax(
            tf.matmul(self.outputs, self.weights) + self.biases
        )

        y = tf.expand_dims(self.y, axis=1)
        self.loss = tf.reduce_mean(
            input_tensor=tf.nn.softmax_cross_entropy_with_logits(
                logits=self.pred, labels=tf.stop_gradient(y)
            )
        )
