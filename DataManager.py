...
class DataManager(object):
    ...
    def generate_overlapRIDs(self, RIDs_list):
        sets = iter(list(map(set, RIDs_list)))
        result = next(sets)
        for s in sets:
        ...
        for i in range(5):
            test_folds.append(
                np.concatenate(
                    (
                        positive_RIDs[
                            i * chunks_positive : (i + 1) * chunks_positive
                        ],
                        negative_RIDs[
                            i * chunks_negative : (i + 1) * chunks_negative
                        ],
                    )
                )
            )
    ...
    def generate_demo(self):
        ...
        # Scaling
        demo["PTGENDER"] = [
            1.0 if i == "Male" else 0.0 for i in demo["PTGENDER"]
        ]
    ...
    def generate_cog(self, max_seq_len):
        ...
        for RID in RIDs:
            ...
            for i in range(max_seq_len):
                if i < seq_length:
                    temp = np.array(
                        record.iloc[i, :][["ADNI_MEM", "ADNI_EF"]].tolist()
                    )
                    temp[np.isnan(temp)] = 0
                else:
                    temp = [0.0 for j in range(2)]
